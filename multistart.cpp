...
void evolvePopulation(Problem* problem) {
    ...
    */
    ...
    for (int j = b_start; j < b_stop - 1; j = j + 2) {
        std::uniform_int_distribution<int> population_distribution(j, population_size - 1);
        int random_parent_1 = population_distribution(generator);
        int random_parent_2 = population_distribution(generator);
